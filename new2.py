...
if movies_file and ratings_file:
    ...
    def compute_diversity_score(recs, tfidf_matrix):
        """Compute diversity as average pairwise cosine distance between recommendations."""
        if len(recs) < 2:
            return 0.0
        vectors = [tfidf_matrix.loc[movie_id].values for movie_id in recs]
        distances = [cosine(v1, v2) for i, v1 in enumerate(vectors) for v2 in vectors[i+1:]]
    ...
    def recommend_similar_movies(query_movie_title, user_id=None, top_n=10, diversity_weight=0.2):
        ...
        # Select top candidates and adjust for diversity
            ...
            if isinstance(recs, str):
                ...
                for i, (title, explanation, pred_rating, score) in enumerate(recs, 1):
                    with st.expander(f"{i}. {title} (Score: {score:.3f})"):
                        st.write(explanation)  # Ensure explanation is always displayed
